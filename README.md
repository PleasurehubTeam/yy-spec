...
- **首次运行（Bootstrap）**：扫描代码库，自动生成 `product.md`（产品定位）、`tech.md`（技术栈）、`structure.md`（项目结构）到 `.kiro/steering/` 目录
...
完整流程在一个会话内完成：
1. 创建规格目录 `.kiro/specs/<feature-name>/`
...
**10 分钟内生成：**
- [requirements.md](.kiro/specs/photo-albums/requirements.md) — 15 条 EARS 格式需求
- [design.md](.kiro/specs/photo-albums/design.md) — 含 Mermaid 图表的架构设计
- [tasks.md](.kiro/specs/photo-albums/tasks.md) — 12 个带依赖关系的实现任务
...
在 `.kiro/settings/` 中自定义模板和规则以匹配你的团队工作流：
