...
- Parse conversation history for `/yy:fix`, `/yy:feature`, `/yy:plan-exec`, or legacy `/kiro:spec-impl` commands
...
## Tool Guidance
- **Conversation parsing**: Extract `/yy:fix`, `/yy:feature`, `/yy:plan-exec`, or legacy `/kiro:spec-impl` patterns from history
...
### Error Scenarios
- **No Implementation Found**: If no implementation commands in history and no `[x]` tasks, report "No implementations detected"
