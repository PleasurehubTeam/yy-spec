...
你不需要学任何 API。但你需要**明确告诉 AI 去操作浏览器**，它才会调用 Playwright。
...
MCP 工具安装后会出现在 AI 的可用工具列表中，但 AI 不会主动使用。你需要明确提到浏览器相关的操作：

- "打开浏览器看看 localhost:3000"
- "用浏览器测试一下登录流程"
- "帮我截图看看页面效果"

只要你的指令涉及"打开页面""截图""点击按钮"等浏览器操作，AI 就会调用 Playwright。
